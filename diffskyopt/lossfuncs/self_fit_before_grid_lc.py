...
class SelfFit:
    ...
    def __init__(self, zmin=0.4, zmax=2.0, lgmp_min=10.5, sky_area_degsq=0.01,
        ...
        hmfsamp_ktrain_fn = f"hmfsamp_{ktrain_fn}"
        ...
        if os.path.exists(hmfsamp_ktrain_fn):
            hmfsamp_ktrain = kdescent.KPretrainer.load(hmfsamp_ktrain_fn)
        else:
            hmfsamp_ktrain = kdescent.KPretrainer.from_training_data(
            ...
            if not RANK:
                hmfsamp_ktrain.save(hmfsamp_ktrain_fn)
        self.kcalc = kdescent.KCalc(ktrain)
        self.kcalc_hmfsamp = kdescent.KCalc(hmfsamp_ktrain)
